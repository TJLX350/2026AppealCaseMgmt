...
(function (global) {
  ...
  function loadNotebookLmLink() {
    try {
      return localStorage.getItem("evich_notebooklm_url") || "";
    } catch (e) { return ""; }
  }
  function saveNotebookLmLink(url) {
    try { localStorage.setItem("evich_notebooklm_url", (url || "").trim()); } catch (e) {}
  }
  ...
  const PUNCHLIST_STATUS = [
    { item: "Dual Owner/Helper + PIN", status: "done" },
    { item: "Stephanie own API keys", status: "done" },
    { item: "Free Toolbox", status: "done" },
    { item: "F83 not appealable + final draft review", status: "done" },
    { item: "Confirm-before-send + cost gate", status: "done" },
    { item: "Training ladder + oral/spar", status: "done" },
    { item: "Issue I packet + baffle", status: "done" },
    { item: "Harm lexicon + Dec 4 analysis", status: "done" },
    { item: "L1–L4 hashing + NL integrity + exports", status: "done" },
    { item: "Mobile-first + Print page/section/selection", status: "done" },
    { item: "Native Share text/email for content", status: "done" },
    { item: "Owner GitHub version panel", status: "done" },
    { item: "Helper 4-tile home", status: "done" },
    { item: "NotebookLM bridge instructions", status: "done" },
    { item: "Meta pre-mortem", status: "done" },
    { item: "Rollout checklist", status: "done" },
    { item: "Full Drive API auto-sync", status: "deferred", note: "Optional; integrity + manual migration is the supported path." },
    { item: "L3 binary hash job (Apps Script)", status: "playbook", note: "Instructions in Owner Tools; you run once when folder stable." },
    { item: "Cryptographic digital signatures (GPG)", status: "deferred", note: "Hashes seal content; GPG optional later." },
    { item: "SMS deadline reminders", status: "deferred", note: "No server in this PWA; use C.A.R. Deadlines + phone calendar." },
    { item: "Hearing transcript page/line pins", status: "owner_source", note: "You add final transcript file to Drive + migrate text." },
    { item: "Exhibit E complete OCR", status: "owner_source", note: "Chunk playbook; last big source file." },
  ];
